...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (typeof req.body._id !== 'string' || !ObjectId.isValid(req.body._id))
    return res.status(HttpStatus.BadRequest).json({ message: '刪除藥局失敗!' });
  ...
  try {
    ...
    if (result.modifiedCount === 0) {
      return res.status(HttpStatus.NotFound).json({ message: '藥局不存在!' });
    }

    res.status(HttpStatus.Ok).json({ message: `已刪除${pharmacy.title}!` });
  } catch (error) {
