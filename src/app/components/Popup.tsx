...
const Popup = ({ title, children, display, onClose }: PopupProps): ReactElement => {
  return (
    <>
      <div
        className={`fixed inset-0 flex items-center justify-center bg-opacity-40 transition-all duration-300 ${display ? 'opacity-100' : 'opacity-0'} ${display ? 'z-10' : '-z-10'} ${display ? 'bg-black' : 'bg-transparent'}`}
        onClick={onClose}
      >
        <div
